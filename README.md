...
### 5. AI 行情助手
- 超管在右上角菜单 → **AI 设置**配置任意 OpenAI 兼容接口（Base URL / API Key / 模型名 + 可选 temperature / 输出上限 / 每人每日次数 / 追加提示词），Key 只存服务端 `server/data/ai-config.json`（不入库），界面回显打码，**保存即生效无需重启**
- 支持多个配置档案：`default` 服务所有用户，其他档案可**绑定指定用户**（如给某用户单独的模型/限额），一个用户只能绑定一个档案
- 配好后右侧出现 3D 悬浮机器人（three.js）：**点击**对话、**拖拽**移动（位置记忆）、**右键**隐藏（菜单可再唤起）
- 聊天窗支持流式输出；每次对话服务端实时注入行情快照（BTC/ETH、成交额前 15、涨跌幅前 6、市场情绪）+ 金融行业规范的系统提示词（结论先行 / 数据引用 / 风险与失效条件 / 免责），可在设置中 ❗ 查看当前生效提示词；欢迎页聚合 Decrypt / Cointelegraph 最新要闻（10 分钟缓存）
